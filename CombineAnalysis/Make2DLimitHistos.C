...
void Make2DLimitHistos(TString signaltype, int analysis=0, bool prefit=false, bool fakedata=false, bool nosigunc=false, bool nobkgunc=false, bool dropsigcont=false, bool correlated=true, TString inputdir = "/hadoop/cms/store/user/haweber/condor/limits/", TString outputdir = "rootfiles/");
...
void Make2DLimitHistos(TString signaltype, int analysis, bool prefit, bool fakedata, bool nosigunc, bool nobkgunc, bool dropsigcont, bool correlated, TString inputdir, TString outputdir){
  ...
  else if(nosigunc)       myoutputdir = myoutputdir + "nosigsyst/";
  else if(nobkgunc)       myoutputdir = myoutputdir + "nobgsyst/";
  ...
  TString outfilename = myoutputdir + "Limits2DHistograms_"+anname+"_"+signaltype+"_prefit.root";
  if(!prefit) outfilename = myoutputdir + "Limits2DHistograms_"+anname+"_"+signaltype+"_postfit.root";
  ...
  for(int stop = mStopLow; stop<=mStopHigh; stop += mStopStep){
    for(int lsp = mLSPLow; lsp<=mLSPHigh; lsp += mLSPStep){
      ...
      limitfilebase = limitfilebase + anname + "_";
	...
	continue;
      ...
      cout << "Limit file exists for analysis " << anname << " and signal " << signalname << endl;
...
TGraph* GetContour(TGraph2D *g, TString name, TGraph *gempty){
  ...
  for(int i = 0; i<glist->GetSize(); ++i){
    ...
    if(Npoints>max_points){
      ...
      max_points = Npoints;
      //sometimes the TGraph doesn't go to MLSP=0 - force it
      if(gnew->GetN()>0){
	double x,y;
	gnew->GetPoint(0,x,y);
	if(y<13.) gnew->SetPoint(0,x,0);
	gnew->GetPoint(gnew->GetN()-1,x,y);
	if(y<13.) { gnew->SetPoint(gnew->GetN(),x,0); }
      }
