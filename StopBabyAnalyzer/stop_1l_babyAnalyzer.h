...
protected: 
	...
	float	ak4jets_rho_;
	TBranch *ak4jets_rho_branch;
	bool ak4jets_rho_isLoaded;
	...
	bool Mlb_isLoaded;
	float	Mlb_lep2_;
	TBranch *Mlb_lep2_branch;
	bool Mlb_lep2_isLoaded;
	float	M3b_;
	TBranch *M3b_branch;
	bool M3b_isLoaded;
	float	M3b_lep2_;
...
void Init(TTree *tree) {
  ...
  tree->SetMakeClass(1);
	...
	ak4jets_rho_branch = 0;
	if (tree->GetBranch("ak4jets_rho") != 0) {
		ak4jets_rho_branch = tree->GetBranch("ak4jets_rho");
		if (ak4jets_rho_branch) {ak4jets_rho_branch->SetAddress(&ak4jets_rho_);}
	}
	...
	Mlb_branch = 0;
	if (tree->GetBranch("Mlb") != 0) {
		Mlb_branch = tree->GetBranch("Mlb");
		if (Mlb_branch) {Mlb_branch->SetAddress(&Mlb_);}
	}
	Mlb_lep2_branch = 0;
	if (tree->GetBranch("Mlb_lep2") != 0) {
		Mlb_lep2_branch = tree->GetBranch("Mlb_lep2");
		if (Mlb_lep2_branch) {Mlb_lep2_branch->SetAddress(&Mlb_lep2_);}
	}
	M3b_branch = 0;
	...
	// load all branches
{
	...
	if (ak4jets_rho_branch != 0) ak4jets_rho();
	...
	float &ak4jets_rho()
	{
		if (not ak4jets_rho_isLoaded) {
			if (ak4jets_rho_branch != 0) {
				ak4jets_rho_branch->GetEntry(index);
			} else { 
				printf("branch ak4jets_rho_branch does not exist!\n");
				exit(1);
			}
			ak4jets_rho_isLoaded = true;
		}
		return ak4jets_rho_;
	}
	...
	float &Mlb()
	{
		if (not Mlb_isLoaded) {
			if (Mlb_branch != 0) {
				Mlb_branch->GetEntry(index);
			} else { 
				printf("branch Mlb_branch does not exist!\n");
				exit(1);
			}
			Mlb_isLoaded = true;
		}
	...
	float &Mlb_lep2()
	{
		if (not Mlb_lep2_isLoaded) {
			if (Mlb_lep2_branch != 0) {
				Mlb_lep2_branch->GetEntry(index);
			} else { 
				printf("branch Mlb_lep2_branch does not exist!\n");
				exit(1);
			}
			Mlb_lep2_isLoaded = true;
		}
		return Mlb_lep2_;
	}
	float &M3b()
...
namespace stop_1l {
	...
	const double &topness();
	const double &Topness_lep2();
	const double &TopnessMod_lep1();
	const double &TopnessMod_lep2();
