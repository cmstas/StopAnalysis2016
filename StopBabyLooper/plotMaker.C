...
void plotMaker( bool plotByGenDecay ){
  ...
  for(int iReg=0; iReg<nRegions; iReg++){
    ...
    for(int iVar=0; iVar<(int)var_list_label.size(); iVar++){
      ...
      // Separate yield histograms for corridor and bulk regions
      if( var_list_label.at(iVar) == "h_yields" ) {
	      if( TString(regionList.at(iReg)).Contains( "corridor" ) && var_rebin_nBins.at(iVar)==27 ) continue;
	      if( TString(regionList.at(iReg)).Contains( "bulk" ) && var_rebin_nBins.at(iVar)==4 ) continue;
      }
