{
  ...
  TH2D *h_data = (TH2D*)f_data->Get("h2__lep1_vs_secondLepType_ge4j");
  h_data->Scale(1.0/h_data->Integral());
  h_data->GetYaxis()->SetRange(1,4);
  h_data->SetTitle("Observed, leading vs second reco lepton flavour, >=4 jets");
  TCanvas *c_data = new TCanvas("c_data", "c_data",10,10,800,1000);
  h_data->Draw("colz text e");
  ...
  TH2D *h_mc = (TH2D*)f_mc->Get("h2__lep1_vs_secondLepType_ge4j");
  h_mc->Scale(1.0/h_mc->Integral());
  h_mc->GetYaxis()->SetRange(1,4);
  h_mc->SetTitle("Monte Carlo, leading vs second reco lepton flavour, >=4 jets");
  TCanvas *c_mc = new TCanvas("c_mc", "c_mc",10,10,800,1000);
  h_mc->Draw("colz text e");
