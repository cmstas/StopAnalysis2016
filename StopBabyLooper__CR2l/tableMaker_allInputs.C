...
int tableMaker_allInputs( std::string f_input_dir="Histos/Nominal/", std::string f_out_name="Output/Tables/yields_allInputs.tex", bool useZeroedAMCNLO=true ){
  ...
  sampleList.push_back( sampleInfo::k_WZ_pythia8 );
  ...
  sampleList.push_back( sampleInfo::k_ZZ_pythia8 );
  ...
  for(int iSys=0; iSys<(int)systematicList.size(); iSys++){
	...
	fprintf(f_out,"c|");
      ...
      // Top Title
      std::string table_title = systematicList[iSys].tex;
	...
	if( sample.isSignalScan ){
	  ...
	  for(int iMassPt=0; iMassPt<(int)massPtList.size(); iMassPt++){
	    ...
	    for(int iGen=0; iGen<(int)genClassyList.size(); iGen++){
	      ...
	      genClassyInfo::genClassyUtil genClassy( genClassyList[iGen] );
		...
		std::string h_name = histogramInfo::getYieldHistoLabel( "yields", genClassy, recoClassyInfo::recoClassyUtil(recoClassyInfo::k_incl), systematicList[iSys] );
	...
	// If not signal scan
	  ...
	  for(int iGen=0; iGen<(int)genClassyList.size(); iGen++){
	    ...
	    for(int iCat=0; iCat<(int)tableList[iTable].size(); iCat++){
	      ...
	      std::string h_name = histogramInfo::getYieldHistoLabel( "yields", genClassyList[iGen], recoClassyInfo::k_incl, systematicList[iSys] );
