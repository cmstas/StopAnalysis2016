...
int tableMaker_summedInputs( std::string f_input_dir="Histos/Nominal/", std::string f_out_name="Output/Tables/yields_summedInputs.tex", bool useZeroedAMCNLO=true ){
  ...
  sampleInfo::sampleUtil data_util( sampleInfo::k_diLepton_2015CD );
  ...
  for(int iSys=0; iSys<(int)systematicList.size(); iSys++){
	...
	for(int iSig=0; iSig<(int)sigList.size(); iSig++){
	  ...
	  for(int iMassPt=0; iMassPt<(int)massPtList.size(); iMassPt++){
	    ...
	    for(int iCat=0; iCat<(int)tableList[iTable].size(); iCat++){
	      ...
	      std::string h_name = histogramInfo::getYieldHistoLabel( "yields", genClassy, recoClassyList[iReco], systematic );
	...
	for(int iCat=0; iCat<(int)tableList[iTable].size(); iCat++){
	  ...
	  std::string h_name = histogramInfo::getYieldHistoLabel( "yields", genClassyInfo::genClassyUtil(genClassyInfo::k_incl), recoClassyList[iReco], systematic );
	...
	for(int iCat=0; iCat<(int)tableList[iTable].size(); iCat++){
	  ...
	  std::string h_name = histogramInfo::getYieldHistoLabel( "yields", genClassyInfo::genClassyUtil(genClassyInfo::k_incl), recoClassyList[iReco], systematicInfo::systematicUtil(systematicInfo::k_nominal) );
