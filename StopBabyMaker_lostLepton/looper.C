...
babyMaker::babyMaker()
{
  ...
  gen_hadtau_daughters = GenParticleTree("hadtau_daughters_");
...
void babyMaker::MakeBabyNtuple(const char* output_name){
  ...
  gen_hadtau_daughters.SetBranches(BabyTree);
...
int babyMaker::looper(TChain* chain, char* output_name, int nEvents, char* path){
  ...
  while ( (currentFile = (TFile*)fileIter.Next()) ) { 
	...
	if(fabs(pfcands_dz().at(ipf)) > 0.2) continue;
	...
	*/
	...
	if( (abs(genps_id_mother().at(genx)) == pdg_tau || abs(genps_id_mother().at(genps_idx_mother().at(genx))) == pdg_tau) && abs(genps_id().at(genx)) != pdg_nue && abs(genps_id().at(genx)) != pdg_el && abs(genps_id().at(genx)) != pdg_numu && abs(genps_id().at(genx)) != pdg_mu && abs(genps_id().at(genx)) != pdg_nutau && abs(genps_id().at(genx)) != pdg_tau && abs(genps_id().at(genx)) != pdg_gamma && abs(genps_id().at(genx)) != pdg_glu && genps_status().at(genx) == 1 ) gen_hadtau_daughters.FillCommon(genx);
	...
	if(abs(genps_id().at(genx)) == pdg_chi_1plus1 && genps_status().at(genx) == 62) StopEvt.mass_chargino = genps_mass().at(genx);
      ...
      StopEvt.HLT_SingleEl = passHLTTriggerPattern("HLT_Ele27_WP85_Gsf_v1");   
      ...
      StopEvt.HLT_MET120Btag = passHLTTriggerPattern("HLT_PFMET120_NoiseCleaned_BTagCSV07_v1");
      StopEvt.HLT_MET120Mu5 = passHLTTriggerPattern("HLT_PFMET120_NoiseCleaned_Mu5_v1"); 
