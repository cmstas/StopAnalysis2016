...
std::vector<bool> selectionInfo::get_selectionResults_SR(int jesType){
  ...
  */
  ...
  temp_result=false;
  if( jesType==1 ){
    if( babyAnalyzer.mindphi_met_j1_j2_jup()>=0.8 ) temp_result = true;
  }
  else if( jesType==-1){
    if( babyAnalyzer.mindphi_met_j1_j2_jdown()>=0.8 ) temp_result = true;
  }
  else{
    if( babyAnalyzer.mindphi_met_j1_j2()>=0.8 ) temp_result = true;
  }
  result.push_back(temp_result);
...
std::vector<bool> selectionInfo::get_selectionResults_CR2l(int jesType, bool inclTau, bool add2ndLepToMet){
  ...
  */
  ...
  // 9) minDPhi(met,j1/j2)>0.8
  temp_result=false;
  if( add2ndLepToMet ){
    if( jesType==1 ){
      if( babyAnalyzer.mindphi_met_j1_j2_rl_jup()>=0.8 ) temp_result = true;
    }
    else if( jesType==-1){
      if( babyAnalyzer.mindphi_met_j1_j2_rl_jdown()>=0.8 ) temp_result = true;
    }
    else{
      if( babyAnalyzer.mindphi_met_j1_j2_rl()>=0.8 ) temp_result = true;
    }
  }
  else{
    if( jesType==1 ){
